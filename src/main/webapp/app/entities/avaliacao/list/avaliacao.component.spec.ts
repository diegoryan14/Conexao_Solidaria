...
import { sampleWithRequiredData } from '../avaliacao.test-samples';
...
describe('Avaliacao Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule, AvaliacaoComponent],
      providers: [
        {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.avaliacaos?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to avaliacaoService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getAvaliacaoIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getAvaliacaoIdentifier).toHaveBeenCalledWith(entity);
